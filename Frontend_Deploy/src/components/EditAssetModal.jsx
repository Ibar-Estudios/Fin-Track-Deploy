...
import { editAssetById } from "../services/assetService";
...
export default function EditAssetModal({ asset, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSuggestions([]);
    try {
      await editAssetById(asset._id, payload);
      setLoading(false);
      onSuccess();
    } catch (error) {
      console.error(
        "❌ Error al editar activo:",
        error.response?.data || error.message
      );
      setLoading(false);
      setMessage("Error al editar el activo.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-ink/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="card w-full max-w-md">
        <h2 className="font-display text-xl font-bold mb-4">Editar activo</h2>
